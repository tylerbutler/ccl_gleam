...
pub fn run_parsing_benchmarks() {
  ...
  let parsing_functions = [
    bench.Function("parse_only", fn(config_text) { ccl_core.parse(config_text) }),
  ]

  bench.run(test_configs, parsing_functions, [bench.Duration(3000)]// 3 second test duration per benchmark
  )
...
pub fn run_construction_benchmarks() {
  ...
  bench.run(construction_configs, parsing_functions, [bench.Duration(3000)])
  |> bench.table([bench.IPS, bench.Min, bench.P(99)])
  |> io.println()
  ...
  bench.run(parsed_entries_configs, construction_functions, [
    bench.Duration(3000),
  ])
  |> bench.table([bench.IPS, bench.Min, bench.P(99)])
  |> io.println()
...
pub fn run_typed_parsing_benchmarks() {
  ...
  bench.run(typed_configs, access_functions, [bench.Duration(3000)])
  |> bench.table([bench.IPS, bench.Min, bench.P(99)])
  |> io.println()
}
