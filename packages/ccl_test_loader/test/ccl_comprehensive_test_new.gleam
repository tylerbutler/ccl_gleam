...
fn print_new_test_overview() {
  ...
  io.println("Property Tests: " <> string.inspect(property_test_count) <> " tests")
...
pub fn comprehensive_property_tests() {
  ...
  io.println("Property Tests: " <> string.inspect(total_tests) <> " total tests")
}
...
fn run_api_test_suite(suite: test_suite_types.NewTestSuite) -> Int {
  let results = list.map(suite.tests, fn(test_case) {
    ...
    let all_passed = case list.length(api_results) {
      0 -> True  // No validations to run
      _ -> list.all(api_results, fn(result) {
        case result {
  ...
  io.println(
    suite.suite <> ": " <> string.inspect(passed) <> "/" <> string.inspect(total) <> " passed"
  )
...
fn run_property_test_suite(suite: test_suite_types.NewTestSuite) -> Int {
  let results = list.map(suite.tests, fn(test_case) {
    ...
    let all_passed = case list.length(property_results) {
      0 -> True  // No validations to run
      _ -> list.all(property_results, fn(result) {
        case result {
