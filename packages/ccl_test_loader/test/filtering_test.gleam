...
fn sample_tests() -> List(ccl_test_loader.TestCase) {
  [
    ...
    create_test_case("comment handling", 2, [
      "feature:comments",
      "function:parse",
    ]),
    create_test_case("object construction", 3, ["function:make-objects"]),
    create_test_case("dotted keys", 3, [
      "feature:dotted-keys",
    ...
    create_test_case("proposed variant test", 1, [
      "function:parse",
      "variant:proposed-behavior",
    ]),
    create_test_case("legacy variant test", 1, [
      "function:parse",
      "variant:legacy-unsupported",
    ]),
  ]
}
...
pub fn by_function_test() {
  ...
  let parse_tests =
    ccl_test_loader.filter_tests(tests, ccl_test_loader.ByFunction("parse"))
  should.equal(4, list.length(parse_tests))
  // basic + comment + 2 variant tests

  let make_objects_tests =
    ccl_test_loader.filter_tests(
      tests,
      ccl_test_loader.ByFunction("make-objects"),
    )
  should.equal(2, list.length(make_objects_tests))
  // object construction + dotted keys

  let get_string_tests =
    ccl_test_loader.filter_tests(
      tests,
      ccl_test_loader.ByFunction("get-string"),
    )
  should.equal(1, list.length(get_string_tests))
  // typed string access

  let get_int_tests =
    ccl_test_loader.filter_tests(tests, ccl_test_loader.ByFunction("get-int"))
  should.equal(1, list.length(get_int_tests))
  // typed int access
}
...
pub fn by_feature_test() {
  ...
  let comment_tests =
    ccl_test_loader.filter_tests(tests, ccl_test_loader.ByFeature("comments"))
  should.equal(1, list.length(comment_tests))

  let dotted_key_tests =
    ccl_test_loader.filter_tests(
      tests,
      ccl_test_loader.ByFeature("dotted-keys"),
    )
...
pub fn variant_filter_test() {
  ...
  let proposed_tests =
    ccl_test_loader.filter_tests(
      tests,
      ccl_test_loader.ByVariant("proposed-behavior"),
    )
  should.equal(1, list.length(proposed_tests))
  // Only the proposed variant test

  let reference_tests =
    ccl_test_loader.filter_tests(
      tests,
      ccl_test_loader.ByVariant("reference-compliant"),
    )
  should.equal(0, list.length(reference_tests))
  // No reference-compliant tests in sample
}
...
pub fn edge_cases_test() {
  ...
  // Non-existent level
  let no_level =
    ccl_test_loader.filter_tests(tests, ccl_test_loader.ByLevel(99))
  ...
  let no_function =
    ccl_test_loader.filter_tests(
      tests,
      ccl_test_loader.ByFunction("nonexistent"),
    )
  ...
  let no_feature =
    ccl_test_loader.filter_tests(
      tests,
      ccl_test_loader.ByFeature("nonexistent"),
    )
  ...
  let no_variant =
    ccl_test_loader.filter_tests(
      tests,
      ccl_test_loader.ByVariant("nonexistent"),
    )
