...
fn demo_node_type_detection(ccl_core_obj: ccl_core.CCL) {
  ...
  let test_paths = [
    "app_name",
    // Should be SingleValue
    "ports",
    // Should be ListValue 
    "single_port",
    // Should be SingleValue (single item list)
    "database",
...
fn demo_smart_access_functions(ccl_core_obj: ccl_core.CCL) {
  ...
  demo_smart_value(ccl_core_obj, "app_name")
  // Should work
  demo_smart_value(ccl_core_obj, "ports")
  // Should suggest get_list()
  demo_smart_value(ccl_core_obj, "database")
  // Should suggest get_nested()

  io.println("\nUsing get_list():")
  demo_get_list(ccl_core_obj, "ports")
  // Should return list
  demo_get_list(ccl_core_obj, "single_port")
  // Should return single-item list
  demo_get_list(ccl_core_obj, "app_name")
  // Should return single-item list
  demo_get_list(ccl_core_obj, "database")
  // Should error

  io.println("\nUsing get_value_or_first():")
  demo_get_value_or_first(ccl_core_obj, "app_name")
  // Single value
  demo_get_value_or_first(ccl_core_obj, "ports")
  // First item of list
  demo_get_value_or_first(ccl_core_obj, "single_port")
  // Single item
...
fn demo_get_list(ccl_core_obj: ccl_core.CCL, path: String) {
  case ccl.get_list(ccl_core_obj, path) {
    Ok(values) ->
      io.println("  " <> path <> " = [" <> string.join(values, ", ") <> "]")
    Error(err) -> io.println("  " <> path <> " -> " <> err)
  }
}
...
fn demo_list_access_patterns(ccl_core_obj: ccl_core.CCL) {
  ...
  case ccl.get_list(ccl_core_obj, "ports") {
    Ok(ports) -> {
      list.each(ports, fn(port) { io.println("  - Configuring port " <> port) })
    }
...
fn demo_error_handling_improvements(ccl_core_obj: ccl_core.CCL) {
  ...
  let test_cases = [
    #("get_smart_value", "ports"),
    // List accessed as single value
    #("get_smart_value", "database"),
    // Object accessed as single value
    #("get_list", "database"),
    // Object accessed as list
    #("get_value_or_first", "database"),
    // Object accessed as value
  ]
  ...
  list.each(test_cases, fn(test_case) {
    ...
    case function_name {
      "get_smart_value" -> {
        case ccl.get_smart_value(ccl_core_obj, path) {
          Ok(value) ->
            io.println("  " <> function_name <> "(" <> path <> ") = " <> value)
          Error(err) ->
            io.println("  " <> function_name <> "(" <> path <> ") -> " <> err)
        }
      }
      "get_list" -> {
        case ccl.get_list(ccl_core_obj, path) {
