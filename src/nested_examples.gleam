...
pub fn get_section(config: NestedConfig, prefix: String) -> List(#(String, String)) {
  dict.to_list(config)
  |> list.filter(fn(pair) {
    let #(key, _) = pair
...
api.cors.origins =
  ...
  case ccl.parse(config_text) {
    Ok(entries) -> {
      ...
      case dict.get(config, "api.cors.origins") {
        Ok(origins) -> {
          ...
          list.each(origin_list, fn(origin) {
            io.println("  - " <> origin)
          })
        }
    ...
    Error(err) -> {
      io.println("Parse error at line " <> string.inspect(err.line) <> ": " <> err.reason)
    }
  }
}
...
pub fn extract_database_config(config: NestedConfig) -> Result(DatabaseConfig, String) {
...
pub fn extract_database_config_proper(config: NestedConfig) -> Result(DatabaseConfig, String) {
  use host <- result.try(dict.get(config, "database.host") |> result.replace_error("Missing database.host"))
  use port <- result.try(dict.get(config, "database.port") |> result.replace_error("Missing database.port"))
  use name <- result.try(dict.get(config, "database.name") |> result.replace_error("Missing database.name"))
  use username <- result.try(dict.get(config, "database.credentials.username") |> result.replace_error("Missing database.credentials.username"))
  use password <- result.try(dict.get(config, "database.credentials.password") |> result.replace_error("Missing database.credentials.password"))
  use min_pool <- result.try(dict.get(config, "database.pool.min_size") |> result.replace_error("Missing database.pool.min_size"))
  use max_pool <- result.try(dict.get(config, "database.pool.max_size") |> result.replace_error("Missing database.pool.max_size"))
