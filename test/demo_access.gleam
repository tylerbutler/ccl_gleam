...
fn demo_list_access(ccl_obj: ccl_core.CCL) {
  ...
  list.each(port_values, fn(port) { io.println("  - Port: " <> port) })
...
fn demo_exploration(ccl_obj: ccl_core.CCL) {
  ...
  io.println(
    "  'simple' exists: " <> string.inspect(ccl_core.has_key(ccl_obj, "simple")),
  )
  io.println(
    "  'database.host' exists: "
    <> string.inspect(ccl_core.has_key(ccl_obj, "database.host")),
  )
  io.println(
    "  'nonexistent' exists: "
    <> string.inspect(ccl_core.has_key(ccl_obj, "nonexistent")),
  )
  ...
  list.each(all_paths, fn(path) { io.println("  - " <> path) })
...
fn demo_error_handling(ccl_obj: ccl_core.CCL) {
  ...
  case ccl_core.get_value(ccl_obj, "database.nonexistent") {
    ...
    Error(err) ->
      io.println("Expected error for 'database.nonexistent': " <> err)
  }
  ...
  case ccl_core.get_value(ccl_obj, "database.credentials.nonexistent") {
    ...
    Error(err) ->
      io.println(
        "Expected error for 'database.credentials.nonexistent': " <> err,
      )
  }
...
pub fn get_database_config(
  ...
  use username <- result.try(ccl_core.get_value(
    ccl_obj,
    "database.credentials.username",
  ))
  use password <- result.try(ccl_core.get_value(
    ccl_obj,
