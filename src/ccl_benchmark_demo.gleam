...
pub fn run_ccl_performance_demo() {
  ...
  io.println("- Small: " <> int.to_string(string.length(small_config)) <> " bytes")
  io.println("- Medium: " <> int.to_string(string.length(medium_config)) <> " bytes")  
  io.println("- Large: " <> int.to_string(string.length(large_config)) <> " bytes")
  io.println("")
...
fn test_parsing_performance(name: String, config_text: String) {
  ...
  case ccl_core.parse(config_text) {
    Ok(entries) -> {
      ...
      io.println("✅ " <> name <> ": " <> int.to_string(entry_count) <> " entries parsed successfully")
...
fn test_object_construction_performance() {
  ...
  let nested_entries_shallow = generate_nested_entries(20, 2)  // 20 keys, 2 levels deep
  let nested_entries_deep = generate_nested_entries(10, 6)     // 10 keys, 6 levels deep
...
fn test_parsing_step(name: String, config_text: String) {
  case ccl_core.parse(config_text) {
    Ok(entries) -> {
      ...
      io.println("   ✅ " <> name <> " → " <> int.to_string(count) <> " entries parsed")
    }
    Error(_) -> io.println("   ❌ " <> name <> " → parsing failed")
  }
}
...
fn test_construction_step(name: String, config_text: String) {
  case ccl_core.parse(config_text) {
    Ok(entries) -> {
      ...
      io.println("   ✅ " <> name <> " → object constructed from " <> int.to_string(count) <> " entries")
    }
