...
services =
  ...
  case ccl.parse(ccl_text) {
    Ok(flat_entries) -> {
      ...
      demo_node_type_detection(ccl_obj)
      demo_smart_access_functions(ccl_obj)
      demo_list_access_patterns(ccl_obj)
      demo_error_handling_improvements(ccl_obj)
    }
    Error(err) -> io.println("Parse error: " <> err.reason)
  }
}

fn demo_node_type_detection(ccl_obj: ccl.CCL) {
  ...
  list.each(test_paths, fn(path) {
    let node_type = ccl.node_type(ccl_obj, path)
...
fn demo_smart_access_functions(ccl_obj: ccl.CCL) {
  io.println("=== 2. Smart Access Functions ===")
  
  // get_smart_value - gives helpful errors for wrong types
  io.println("Using get_smart_value():")
  demo_smart_value(ccl_obj, "app_name")      // Should work
  demo_smart_value(ccl_obj, "ports")         // Should suggest get_list()
  demo_smart_value(ccl_obj, "database")      // Should suggest get_nested()
  
  io.println("\nUsing get_list():")
  demo_get_list(ccl_obj, "ports")            // Should return list
  demo_get_list(ccl_obj, "single_port")      // Should return single-item list
  demo_get_list(ccl_obj, "app_name")         // Should return single-item list
  demo_get_list(ccl_obj, "database")         // Should error
  
  io.println("\nUsing get_value_or_first():")
  demo_get_value_or_first(ccl_obj, "app_name")    // Single value
  demo_get_value_or_first(ccl_obj, "ports")       // First item of list
  demo_get_value_or_first(ccl_obj, "single_port") // Single item
  
  io.println("")
}

fn demo_smart_value(ccl_obj: ccl.CCL, path: String) {
  case ccl.get_smart_value(ccl_obj, path) {
    Ok(value) -> io.println("  " <> path <> " = " <> value)
    Error(err) -> io.println("  " <> path <> " -> " <> err)
  }
}

fn demo_get_list(ccl_obj: ccl.CCL, path: String) {
  case ccl.get_list(ccl_obj, path) {
    Ok(values) -> io.println("  " <> path <> " = [" <> string.join(values, ", ") <> "]")
    Error(err) -> io.println("  " <> path <> " -> " <> err)
  }
}

fn demo_get_value_or_first(ccl_obj: ccl.CCL, path: String) {
  case ccl.get_value_or_first(ccl_obj, path) {
    Ok(value) -> io.println("  " <> path <> " = " <> value <> " (first/only)")
    Error(err) -> io.println("  " <> path <> " -> " <> err)
  }
}

fn demo_list_access_patterns(ccl_obj: ccl.CCL) {
  io.println("=== 3. Common List Access Patterns ===")
  
  // Pattern 1: Process all items in a list
  io.println("Processing all ports:")
  case ccl.get_list(ccl_obj, "ports") {
    Ok(ports) -> {
  ...
  case ccl.get_value_or_first(ccl_obj, "ports") {
...
fn demo_error_handling_improvements(ccl_obj: ccl.CCL) {
