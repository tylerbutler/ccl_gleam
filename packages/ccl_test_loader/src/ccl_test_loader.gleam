...
pub type TestCase {
  TestCase(name: String, input: String, expected: List(Entry), meta: TestMeta)
}
...
fn test_case_decoder() -> decode.Decoder(TestCase) {
  ...
  decode.one_of(standard_test_case_decoder(), or: [
    object_construction_test_case_decoder(),
  ])
}
  ...
  case filter {
    ...
    ByTag(tag) ->
      list.filter(tests, fn(test_case) {
        list.contains(test_case.meta.tags, tag)
      })
    ByName(name) ->
      list.filter(tests, fn(test_case) { string.contains(test_case.name, name) })
  }
}

/// Load and filter test suite in one step
pub fn load_filtered_tests(
  file_path: String,
  filter: TestFilter,
...
fn test_dotted_key_access(
  ccl: CCL,
  dotted_entries: List(Entry),
) -> Result(Nil, String) {
  case dotted_entries {
    [] -> Ok(Nil)
    // No dotted keys to test
    [first_entry, ..rest] -> {
      ...
      case ccl_core.get_value(ccl, first_entry.key) {
        Ok(value) -> {
          case value == first_entry.value {
            True -> test_dotted_key_access(ccl, rest)
...
pub fn run_test_suite_with_parser(
  file_path: String,
  parse_fn: fn(String) -> Result(List(Entry), e),
) -> TestSuiteResult {
...
pub fn print_test_suite_result(result: TestSuiteResult) -> Nil {
  ...
  io.println(
    "Total: "
    <> string.inspect(result.total)
    <> ", Passed: "
    <> string.inspect(result.passed)
    <> ", Failed: "
...
pub fn ccl_core_test_paths() -> List(String) {
  [
    "/home/tylerbu/code/claude-workspace/ccl-test-data/tests/essential-parsing.json",
    // Level 1
    "/home/tylerbu/code/claude-workspace/ccl-test-data/tests/comprehensive-parsing.json",
    // Level 1 
    "/home/tylerbu/code/claude-workspace/ccl-test-data/tests/comments.json",
    // Level 2
    "/home/tylerbu/code/claude-workspace/ccl-test-data/tests/processing.json",
    // Level 2
    "/home/tylerbu/code/claude-workspace/ccl-test-data/tests/object-construction.json",
    // Level 3
    "/home/tylerbu/code/claude-workspace/ccl-test-data/tests/dotted-keys.json",
    // Level 3
    "/home/tylerbu/code/claude-workspace/ccl-test-data/tests/errors.json",
    // All levels
  ]
}
...
pub fn ccl_package_test_paths() -> List(String) {
  [
    "/home/tylerbu/code/claude-workspace/ccl-test-data/tests/typed-access.json",
    // Level 4
    "/home/tylerbu/code/claude-workspace/ccl-test-data/tests/pretty-print.json",
    // Integration
  ]
}

/// Run all test suites for CCL Core with a provided parser
pub fn run_ccl_core_comprehensive_tests(
  parse_fn: fn(String) -> Result(List(Entry), e),
  ...
  let results =
    list.map(ccl_core_test_paths(), fn(path) {
  ...
  let total_passed =
    list.fold(results, 0, fn(acc, result) { acc + result.passed })
  let total_failed =
    list.fold(results, 0, fn(acc, result) { acc + result.failed })
